...
import { HardHat, Wrench, Plus, Phone, Mail, Trash2, Loader2, CheckCircle2, MapPin, Calendar, Banknote, AlertCircle, MessageCircle } from 'lucide-react'
...
function ArtigianoCard({
  a,
  myEmail,
  proId,
}: {
  a: ManodoperaAvailability
  myEmail: string
  proId?: string
}) {
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              {proId ? (
                <Link
                  href={`/professionisti/${proId}`}
                  target="_blank"
                  className="font-bold text-slate-900 text-sm hover:text-orange-600 hover:underline transition-colors"
                >
                  {a.nome}
                </Link>
              ) : (
          ...
          <div className="flex flex-col gap-1.5 flex-shrink-0">
            <button
              ...
              className="text-xs font-semibold bg-orange-50 text-orange-600 border border-orange-200 px-3 py-2 rounded-xl hover:bg-orange-100 transition-colors"
            >
              {open ? 'Chiudi' : 'Contatta'}
            </button>
            <button
              onClick={() => setChatOpen(true)}
              className="text-xs font-semibold bg-slate-50 text-slate-600 border border-slate-200 px-3 py-2 rounded-xl hover:bg-slate-100 transition-colors flex items-center gap-1 justify-center"
            >
              <MessageCircle className="w-3 h-3" /> Chatta
            </button>
          </div>
        </div>
        {open && (
...
function RichiestaCard({ r, myEmail }: { r: ManodoperaRequest; myEmail: string }) {
  const [open, setOpen] = useState(false)
  const [chatOpen, setChatOpen] = useState(false)
  return (
    <>
      <div className="bg-white border border-slate-200 rounded-2xl p-4 hover:border-orange-300 transition-colors">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex flex-col gap-1.5 flex-shrink-0">
            <button
              onClick={() => setOpen(!open)}
              className="text-xs font-semibold bg-slate-800 text-white px-3 py-2 rounded-xl hover:bg-slate-700 transition-colors"
            >
...
export default function ManodoperaSection({
  ...
  emailToProId: Record<string, string>
}) {
          ...
          <section>
            ...
            {artigianiDisponibili.length === 0 ? (
              ...
              <div className="space-y-3">
                {artigianiDisponibili.map((a) => (
                  <ArtigianoCard key={a.id} a={a} myEmail={pro.email} proId={emailToProId[a.email]} />
                ))}
              </div>
            )}
          </section>
        ...
        <div className="space-y-6">
          ...
          <section>
            ...
            {richiesteAperte.length === 0 ? (
              ...
              <div className="space-y-3">
                {richiesteAperte.map((r) => <RichiestaCard key={r.id} r={r} myEmail={pro.email} />)}
              </div>
            )}
          </section>
