...
import {
  ...
  Zap, CheckCircle2, Crown, ShieldCheck,
} from 'lucide-react'
...
function DeltaBadge({ current, previous, label }: { current: number; previous: number; label: string }) {
  ...
  return (
    <div className="bg-slate-50 rounded-2xl border border-slate-200 p-5">
      <div className="text-3xl font-extrabold text-slate-800 mb-1">{current}</div>
...
export default async function DashboardPage() {
  ...
  if (!pro) {
    return (
      <div className="max-w-lg">
        <h1 className="text-2xl font-extrabold text-slate-800 mb-6">Dashboard</h1>
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 flex items-start gap-4">
          <AlertCircle className="w-6 h-6 text-amber-500 flex-shrink-0 mt-0.5" />
          <div>
            <p className="font-semibold text-slate-800 mb-1">Profilo non trovato</p>
  ...
  const isPro = (pro.plan_type ?? 'free') === 'pro'
  ...
  return (
    <div className="max-w-4xl space-y-10">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatCard
            ...
            sub="Non lette"
            color={nuoveRichieste > 0 ? 'text-orange-500' : 'text-slate-800'}
          />
          <StatCard
            label="In attesa risposta"
            value={inAttesa}
            sub="Dal cliente"
            color={inAttesa > 0 ? 'text-blue-600' : 'text-slate-800'}
          />
          <StatCard
            label="Confermati"
            value={confermatiMese}
            sub="Questo mese"
            color={confermatiMese > 0 ? 'text-green-600' : 'text-slate-800'}
          />
          <StatCard
            label="Media recensioni"
            value={mediaStelle ? `${mediaStelle.toFixed(1)} ★` : '—'}
            sub={`su ${pro.review_count} recens.`}
            color={mediaStelle && mediaStelle >= 4 ? 'text-amber-500' : 'text-slate-800'}
          />
        </div>
      </div>
      ...
      {/* ── COMPLETEZZA PROFILO ─────────────────────────────── */}
      <section>
        <h2 className="text-lg font-bold text-slate-800 mb-4">Completezza profilo</h2>
        <div className="bg-slate-50 rounded-2xl border border-slate-200 p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-semibold text-slate-700">{completePct}% completato</span>
            {completePct === 100 && (
          ...
          <div className="w-full bg-slate-200 rounded-full h-3 mb-4 overflow-hidden">
            <div
              ...
              style={{
                width: `${completePct}%`,
                background: completePct === 100 ? '#22c55e' : completePct >= 60 ? '#f97316' : '#ef4444',
              }}
            />
          </div>
          {missing.length > 0 ? (
            <div className="space-y-2">
              ...
              {missing.map((item) => (
                <Link
                  key={item.label}
                  href={item.href}
                  className="flex items-center justify-between gap-3 text-sm text-slate-600 hover:text-orange-500 group"
                >
                  ...
                  <span className="flex items-center gap-1 text-xs text-orange-500 group-hover:underline">
                    +{item.pct}% <ArrowRight className="w-3 h-3" />
                  </span>
                </Link>
              ))}
            </div>
          ) : (
            <p className="text-sm text-green-600 font-medium">Ottimo! Il tuo profilo è al 100%.</p>
          )}
        </div>
      </section>

      {/* ── RECENSIONI ──────────────────────────────────────── */}
      <ReviewsSection reviews={reviews} avgRating={mediaStelle} proName={pro.ragione_sociale} />

      {/* ── STATISTICHE ─────────────────────────────────────── */}
      <section>
        <h2 className="text-lg font-bold text-slate-800 mb-4">Statistiche</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <DeltaBadge current={viewsThisWeek} previous={viewsLastWeek} label="Visualizzazioni profilo questa settimana" />
          <DeltaBadge current={richiesteThisMo} previous={richiesteLastMo} label="Richieste ricevute questo mese" />
        </div>
      </section>

      {/* ── PIANO ───────────────────────────────────────────── */}
      <section>
        <h2 className="text-lg font-bold text-slate-800 mb-4">Il tuo piano</h2>
        <div className="bg-slate-50 rounded-2xl border border-slate-200 p-5">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                {isPro ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 bg-slate-200 text-slate-600 font-semibold text-sm px-3 py-1 rounded-full">
                    Piano Free
                  </span>
                )}
              </div>
          ...
          {!isPro && (
            <div className="mt-5 pt-5 border-t border-slate-200 grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <p className="font-semibold text-orange-500 mb-2">Pro ✦</p>
