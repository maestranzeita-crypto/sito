...
function getInitialCustom(certs: string[]): string {
  const custom = certs.find((c) => !PREDEFINED_IDS.includes(c))
  return custom ?? ''
}
...
export default function CertificazioniSection({ pro }: { pro: Professional }) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        await saveCertificazioni(certs)
        setSaved(true)
  ...
  return (
    <div id="cert">
      <h3 className="text-sm font-semibold text-slate-700 uppercase tracking-wider mb-1.5">
        Certificazioni
      </h3>
      <p className="text-xs text-slate-400 mb-4">
        Le certificazioni selezionate appariranno come badge sul tuo profilo pubblico.
      </p>
          ...
          <div className="flex-1">
            ...
            {altroChecked && (
              <input
                ...
                className="mt-2 w-full px-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
              />
            )}
          </div>
        </label>
      ...
      <button
        ...
        onClick={handleSave}
        disabled={isPending}
        className="flex items-center gap-2 px-5 py-2 text-sm font-semibold bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white rounded-xl transition-colors"
      >
