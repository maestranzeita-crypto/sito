...
function isValidGmbUrl(url: string): boolean {
  if (!url.trim()) return true // empty is valid (means remove)
  try {
    const u = new URL(url.trim())
    return GMB_PATTERNS.some((p) => u.href.includes(p))
  } catch {
    return false
  }
}
...
export default function GmbSection({ pro }: { pro: Professional }) {
  ...
  function handleSave() {
    ...
    if (trimmed && !isValidGmbUrl(trimmed)) {
      setError('Il link deve provenire da maps.google.com, g.page o goo.gl/maps')
      return
    }
    setError('')
    setSaved(false)
    ...
    <div id="gmb">
      ...
      <div className="flex gap-2 mb-3">
        ...
        <button
          ...
          onClick={handleSave}
          disabled={isPending}
          className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold bg-orange-500 hover:bg-orange-600 disabled:opacity-50 text-white rounded-xl transition-colors flex-shrink-0"
        >
      ...
      {saved && (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-4 py-2.5 text-sm text-green-700 mb-2">
          ...
          Link Google My Business salvato.
        </div>
      )}
